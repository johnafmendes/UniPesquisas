...
//1 - pega a lista de empresas ativas
	...
	if (mysql_num_rows($DBEmpresas)!=0){
	   while($registroEmpresa = mysql_fetch_array ($DBEmpresas)){
           ...
           $query = "SELECT c.idcandidato, c.email, em.idemailmarketing, emc.idemailmarketingcurso, c.password, c.nome ";
           $query .= "FROM candidato c ";
           $query .= "INNER JOIN candidatocurso cc1 ON c.idcandidato=cc1.idcandidato ";
           $query .= "INNER JOIN emailmarketingcurso emc ON emc.idcurso=cc1.idcurso ";
           $query .= "INNER JOIN emailmarketing em ON emc.idemailmarketing=em.idemailmarketing ";
           ...
           $query .= "NOT IN (SELECT cc.idcandidato ";
           $query .= "FROM candidatoemcurso cc ";
           $query .= "INNER JOIN emailmarketingcurso emc1 ON emc1.idemailmarketingcurso=cc.idemailmarketingcurso ";
           $query .= "WHERE cc.idemailmarketingcurso = emc.idemailmarketingcurso) AND ce.receberemail = 1 AND ";
           $query .= "ce.idempresa = ". $registroEmpresa['idempresa'] . " AND IF (emc.idstatuscandidato IS NOT NULL, ";
           $query .= "ce.idstatuscandidato=emc.idstatuscandidato, ce.idstatuscandidato IN (SELECT idstatuscandidato ";
	       ...
	       if (mysql_num_rows($DBEmailCandidatos)!=0){
	          ...
	          while($registroCandidato = mysql_fetch_array ($DBEmailCandidatos)){
	              ...
	              if(!$mail->Send()) {
		             ...
		             $query = "INSERT INTO candidatoemcurso ";
				     $query .= "(idcandidato, idemailmarketingcurso) ";
				     $query .= "Values (".$registroCandidato['idcandidato'].", ".$registroCandidato['idemailmarketingcurso'].")";

				     //echo $query;
				     $DBCandidatoEMCurso = mysql_query ($query, $DB);
				     if (!$DBCandidatoEMCurso) {
...
include("enviarEmailMarketingAlternativa.php");
?>
